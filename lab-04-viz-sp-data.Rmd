...
***There are  `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents the data for 1 Denny's restaurant. The variables are `r names(dennys)`.***

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
***There are  `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents the data for 1 La Quinta's Inn. The variables are `r names(laquinta)`.***
...
3.  Take a look at the websites that the data come from (linked above).
    ...
    What about Denny's?
***Most of the La Quinta's are located in the Americas, but there are a few others around the world. Denny's is only located in the US.***
...
***To find La Quinta's or Denny's outside of the US, you can look at the state variable and compare it to the US states database.***
...
***No, there are none outside of the US.***
...
***The state with the fewest Denny's restaurants is Delaware with only 1 restaurant in the state. There are 403 Denny's in California, the highest number of Denny's in a state. For La Quinta's, there are tons of them in Texas, but in Maine there is only one. The other places with only one La Quinta are from some of the other countries that we stated earlier. This result didn't surprise me that much. Given that the name sounds Spanish, and a bunch of them are in Texas this doesn't strike me as odd because Texas borders Mexico.
...
***The joke isn't really true in North Carolina. There are very few overlapping Denny's and La Quinta locations.***
```{r}
dn_lq %>%
  filter(state == "NC") %>%
ggplot(dn_lq, mapping = aes(x = longitude, y = latitude, color = establishment)) +
...
12. Now filter the data for observations in Texas only, and recreate the plot, with an appropriate `alpha` level.
    Visually, does Mitch Hedberg's joke appear to hold here?
***The joke seems to be accurate in the state of Texas because there are many overlapping Denny's and La Quinta locations. They seem to be in the major cities across Texas.***
```{r}
dn_lq %>%
  filter(state == "TX") %>%
ggplot(dn_lq, mapping = aes(x = longitude, y = latitude, color = establishment)) +
